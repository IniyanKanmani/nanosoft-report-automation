...
def process_files():
    ...
    new_order_sku_quantity = order_sku_quantity.groupby(['sku'], as_index=False).sum()

    refund_sku_quantity_tax_total = data.loc[data['type'] == 'Refund'][['sku', 'quantity', 'product sales tax', 'total']]
    new_refund_sku_quantity_tax_total = refund_sku_quantity_tax_total.groupby(['sku'], as_index=False).sum()
    ...
    while ws1.cell(row=i, column=1).value != None:
        if ws1.cell(row=i, column=1).value in new_order_sku_quantity['sku'].unique():
            ws1.cell(row=i, column=5).value = new_order_sku_quantity.loc[new_order_sku_quantity['sku'] == ws1.cell(row=i, column=1).value]['quantity'].item()
        i = i + 1
    ...
    while ws1.cell(row=i, column=1).value != None:
        if ws1.cell(row=i, column=1).value in new_refund_sku_quantity_tax_total['sku'].unique():
            ws1.cell(row=i, column=14).value = new_refund_sku_quantity_tax_total.loc[new_refund_sku_quantity_tax_total['sku'] == ws1.cell(row=i, column=1).value]['quantity'].item()
        i = i + 1
    ...
    while ws1.cell(row=i, column=1).value != None:
        if ws1.cell(row=i, column=1).value in new_refund_sku_quantity_tax_total['sku'].unique():
            ws1.cell(row=i, column=6).value = new_refund_sku_quantity_tax_total.loc[new_refund_sku_quantity_tax_total['sku'] == ws1.cell(row=i, column=1).value]['product sales tax'].item() * -1
        i = i + 1
    ...
    while ws1.cell(row=i, column=1).value != None:
        if ws1.cell(row=i, column=1).value in new_refund_sku_quantity_tax_total['sku'].unique():
            ws1.cell(row=i, column=7).value = new_refund_sku_quantity_tax_total.loc[new_refund_sku_quantity_tax_total['sku'] == ws1.cell(row=i, column=1).value]['total'].item() * -1
        i = i + 1
